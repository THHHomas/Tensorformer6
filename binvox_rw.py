...
def write(array, fp):
    ...
    if array.ndim==2:
        ...
        dense_voxel_data = sparse_to_dense(array, dims)
    else:
        dense_voxel_data = array
    fp = open(fp, "wb")

    fp.write(b'#binvox 1\n')
    fp.write(bytes('dim '+' '.join(map(str, dims))+'\n', encoding = "utf8"))
    fp.write(bytes('translate '+' '.join(map(str, translate))+'\n', encoding = "utf8"))
    fp.write(bytes('scale '+str(scale)+'\n', encoding = "utf8"))
    fp.write(bytes('data\n', encoding = "utf8"))
    ...
    voxels_flat = voxels_flat.astype(np.int)
    ...
    for c in voxels_flat:
        if c==state:
            ...
            if ctr==127:
                fp.write(bytes(chr(state), encoding = "utf8"))
                fp.write(bytes(chr(ctr), encoding = "utf8"))
                ctr = 0
        else:
            flag = 1
